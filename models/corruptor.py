...
"""
this script implements the corruptor, which is based on BART and T5. 
During training, the corruptor aims to reconstruct the masked false claims. 
During training, the input format is as follows:
    Encoder Input: masked false claim, evidence
    Decoder output: false claim

During inference, the corruptor will learn to inject factual errors into the correct claim.
During inference, the input format is as follows:
    Encoder Input: masked correct claim, evidence
    Decoder output: false claim
"""
...
class Seq2SeqDataset(Dataset):
    ...
    def __init__(self, filename, tokenizer, max_src_len=None, max_tgt_len=None, 
                 ...
                 mask_ratio=0.15, mask_strategy='random', mask_granularity='token', merge_mask=False):
        """
        Args:
            ...
            mask_ratio (float, optional): The mask ratio for the source claim.
            mask_strategy (str, optional): The mask strategy for the source claim.
            mask_granularity (str): The mask granularity for the source claim, only effective when mask_strategy=random.
        ...
        self.data_list = []
        selected_label = "SUPPORTS" if self.inference else "REFUTES"
        
        with open(filename, 'r') as fr:
            for line in fr:
                instance = json.loads(line)
                if instance["verdict"] != selected_label:
                        continue
    ...
    def prepare_src(self, instance):
        ...
        masked_src_text = mask(src_text=src_text, evidence=evidence, tokenizer=self.tokenizer, 
                            mask_token='*', mask_ratio=self.mask_ratio, mask_strategy=self.mask_strategy, 
                            mask_granularity=self.mask_granularity, merge_mask=self.merge_mask)
...
def mask(src_text=None, evidence=None, tokenizer=None, 
         mask_token='*', mask_ratio=None, mask_strategy=None, 
         mask_granularity=None, merge_mask=None):
    """
    Use different strategy and mask ratio to mask the src text
    """
    if mask_granularity=='subword':
        src_tokens = tokenizer.tokenize(src_text)
        evidence_tokens = tokenizer.tokenize(evidence)
    elif mask_granularity=='word':
        src_tokens = nltk.tokenize.word_tokenize(src_text)
        evidence_tokens = nltk.tokenize.word_tokenize(evidence)
    else:
        raise ValueError('')
    # print(src_tokens)
    ...
    if mask_granularity=='subword':
        masked_src_text = tokenizer.convert_tokens_to_string(src_tokens)
    elif mask_granularity=='word':
        masked_src_text = " ".join(src_tokens)
    else:
        raise ValueError("")
...
def generate(model, tokenizer, test_dataloader, args):
    ...
    prefix += f'_{args.mask_strategy}_{args.mask_granularity}'
    if args.mask_strategy == 'random':
        prefix += f'_{args.mask_ratio}'
    if args.merge_mask:
        prefix += '_merge-mask'
...
def predict(model, tokenizer, args):
    ...
    test_dataset = Seq2SeqDataset(args.test_file, tokenizer,
                                    ...
                                    mask_ratio=args.mask_ratio, mask_strategy=args.mask_strategy, 
                                    mask_granularity=args.mask_granularity, merge_mask=args.merge_mask
                                )
...
def get_parameter():
    ...
    parser.add_argument('--use_evidence', action='store_true', help='whether use evidences to revise the original claim.')
    parser.add_argument('--use_gold_evidence', action='store_true', help='whether use gold or retrieved evidences.')
    parser.add_argument('--num_evidence', type=int, default=1, help='the number of evidences used to revise the original claim.')
    
    parser.add_argument('--use_mutation_type', action='store_true', help='whether use the mutation type as input.')
    ...
    parser.add_argument('--mask_granularity', type=str, default='subword', choices=['subword', 'word'], help='The mask granularity for the source claim, only effective when mask_strategy=random.')
    ...
    dir_prefix += f'_{args.mask_strategy}_{args.mask_granularity}'
    if args.mask_strategy == 'random':
        dir_prefix += f'_{args.mask_ratio}'
    if args.merge_mask:
    ...
    if args.output_dir is None:
        if args.do_train:
            args.output_dir = f'../checkpoints_corruptor/{dir_prefix}'
        else:
            args.output_dir = args.model_path
    args.tensorboard_dir = f'../tensorboard_log_corruptor/{dir_prefix}'
