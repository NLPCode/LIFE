...
python -u -m torch.distributed.launch --nproc_per_node=2 --master_port=9536  \
    ...
    --use_evidence true --use_gold_evidence false --num_evidence 2 \
    --fp16 --do_train 
...
python -u -m torch.distributed.launch --nproc_per_node=2 --master_port=9536  \
    ../models/fact_verifier.py \
    --validation_file $DATA_DIR/dev.jsonl \
    --model_name_or_path ../checkpoints_verifier/roberta-base/${DATA_NAME}_seed42_lr2e-05_2-retrieved-evidence \
    --use_evidence true --use_gold_evidence false --num_evidence 2 \
    --resume --fp16 --do_eval
